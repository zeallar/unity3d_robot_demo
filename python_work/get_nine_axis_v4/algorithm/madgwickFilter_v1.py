...
class MadgwickAHRS:
    ...
    def quaternion_to_euler_v2(self) -> Tuple[float, float, float]:
        w, x, y, z = self.q
        ...
        # yaw (z-axis rotation)
        if abs(pitch) < math.pi / 2 - 1e-5:  # Avoid gimbal lock region
            siny_cosp = 2 * (w * z + x * y)
            cosy_cosp = 1 - 2 * (y * y + z * z)
            yaw = math.atan2(siny_cosp, cosy_cosp)
        else:
            # When pitch is near ±90 degrees, yaw is not well-defined
            yaw = 0.0
