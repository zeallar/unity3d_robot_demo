...
{
    private float sampleFreq = 500.0f; // sample frequency in Hz
    ...
    private float pitch = 0.0f, roll = 0.0f, yaw = 0.0f;

    // Function to calculate the inverse square root
    private float invSqrt(float x)
    {
        return 1.0f / Mathf.Sqrt(x); // Using Mathf.Sqrt instead of the fast inverse square root
    }

    // Update method for AHRS algorithm
    ...
    // Update method for IMU algorithm
    ...
    {
        roll = Mathf.Atan2(2.0f * (q0 * q1 + q2 * q3), 1.0f - 2.0f * (q1 * q1 + q2 * q2)) * Mathf.Rad2Deg;
        pitch = Mathf.Asin(2.0f * (q0 * q2 - q3 * q1)) * Mathf.Rad2Deg;
        yaw = Mathf.Atan2(2.0f * (q0 * q3 + q1 * q2), 1.0f - 2.0f * (q2 * q2 + q3 * q3)) * Mathf.Rad2Deg;
    }

    // Method to get the current orientation as a quaternion
    public Quaternion GetQuaternion()
    {
        return new Quaternion(q1, q2, q3, q0);
    }

    // Method to get the current Euler angles
