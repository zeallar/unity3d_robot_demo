...
class MadgwickAHRS:
    ...
    def quaternion_to_euler(self) -> Tuple[float, float, float]:
        ...
        # Apply cumulative yaw adjustment
        if self.prev_euler is not None:
            prev_yaw = self.prev_euler[2]
            delta_yaw = yaw - prev_yaw

            # Normalize the delta_yaw to be within the range [-pi, pi]
        ...
        euler = (roll, pitch, self.cumulative_yaw)
        
        # Apply smoothing if previous Euler angles are available
        if self.prev_euler is not None:
            prev_roll, prev_pitch, prev_cumulative_yaw = self.prev_euler
            
            # Smoothing factor, can be adjusted
            alpha = 0.9
            roll = alpha * prev_roll + (1 - alpha) * roll
            pitch = alpha * prev_pitch + (1 - alpha) * pitch
            self.cumulative_yaw = alpha * prev_cumulative_yaw + (1 - alpha) * self.cumulative_yaw
