...
class MadgwickAHRS:
    ...
    def quaternion_to_euler(self) -> Tuple[float, float, float]:
        ...
        #     # Normalize the delta_euler to be within the range [-pi, pi]
        #     for i in range(3):
        #         if delta_euler[i] > math.pi:
        #             delta_euler[i] -= 2 * math.pi
        #         elif delta_euler[i] < -math.pi:
        #             delta_euler[i] += 2 * math.pi

        #     self.cumulative_euler += delta_euler
        # else:
        #     self.cumulative_euler = euler
        
        # self.prev_euler = euler
        
        # # Apply smoothing if previous Euler angles are available
        # if self.prev_euler is not None:
        #     alpha = 0.9
        #     self.cumulative_euler = alpha * self.prev_euler + (1 - alpha) * self.cumulative_euler
        
        #return tuple(self.cumulative_euler)
        return tuple(euler)
